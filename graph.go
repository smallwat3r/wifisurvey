...
func gnuplotScript(records [][]string, pdf string, meta map[string]string) string {
	...
	// labels at the bar ends stay inside the plot, the left so the first AP box
	// isn't clipped by the y-axis. 8% of the span suits a landscape page; a small
	// floor covers very short surveys.
	pad := int64(float64(lastX-firstX) * 0.08)
	if pad < 30 {
		pad = 30
	}
